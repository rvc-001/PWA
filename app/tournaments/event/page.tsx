...
function PersonChip({ name }: { name: string }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-[#e7e7e7] bg-[#f8f8f8] px-3 py-2 text-sm dark:border-[#806ea8] dark:bg-[#4a396f]">
...
export default function TournamentEventPage() {
  ...
  return (
    <div className="min-h-screen bg-[#ececec] pb-24 text-[#242424] dark:bg-[#3b2a5f] dark:text-[#f4f1ff]">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-white px-3 py-2 text-[#2d2d2d] dark:border dark:border-[#68558f] dark:bg-[#4a396f] dark:text-[#f5f2ff]">
              <div className="flex items-center gap-2">
                <div className="grid h-7 w-7 place-content-center rounded-full border border-[#e2e2e2] text-[#ff7a1a]"><UsersIcon size={14} /></div>
      ...
      <div className="grid grid-cols-2 border-y border-[#d1d1d1] bg-[#efefef] dark:border-[#6a5791] dark:bg-[#3f2d63]">
        <Link href={routes.tournamentDetail(id)} className="grid h-10 place-content-center text-lg font-semibold text-[#979797] dark:text-[#b8aed2]">About</Link>
        ...
        {mockEvents.map((event) => {
          ...
          return (
            <RegistrationEventCard key={event.id} event={event} isSelected={selected} onSelect={() => toggleSelection(event)} onDeselect={() => toggleSelection(event)}>
              {showPairBox ? (
                <div className="rounded-2xl border border-[#dddddd] bg-[#f2f2f2] p-3 dark:border-[#76639e] dark:bg-[#54447c]">
                  {pairStep === "adding" ? (
                    <>
                      <p className="text-xl font-semibold">Add your partner</p>
                      <input placeholder="Enter partner's Phone No." className="mt-2 h-10 w-full rounded-lg border border-[#e0e0e0] bg-white px-3 text-sm outline-none dark:border-[#806ea8] dark:bg-[#4a396f]" />
                      <p className="mt-1 flex items-start gap-1 text-xs text-[#8a8a8a] dark:text-[#d2cae6]"><InfoIcon size={11} className="mt-0.5" />Your partner must be registered on the app to enroll.</p>
                      <button onClick={() => setPairStep("invited")} className="mt-2 h-9 w-full rounded-full border border-[#ff7a1a] text-base font-semibold text-[#ff7a1a]">Add Partner</button>
                    </>
                  ) : null}
                  ...
                  {pairStep === "invited" ? (
                    <>
                      <p className="text-xl font-semibold">Add your partner</p>
                      <div className="mt-2 flex items-center justify-between rounded-lg border border-[#e2e2e2] bg-[#f7f7f7] p-2 text-sm dark:border-[#806ea8] dark:bg-[#4a396f]">
                      ...
                      <button onClick={() => setPairStep("pairing")} className="mt-2 h-9 w-full rounded-full border border-[#ff7a1a] text-base font-semibold text-[#ff7a1a]">Continue</button>
                    </>
                  ) : null}
                  ...
                  {pairStep === "pairing" ? (
                    <>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2"><PersonChip name="You" /><button onClick={() => setPairStep("adding")} className="flex items-center justify-center gap-1 rounded-lg bg-[#ffd9d9] px-3 py-2 text-sm text-[#ef4444]"><TrashIcon size={12} />Remove</button></div>
                      <button onClick={() => setPairStep("paired")} className="mt-2 h-9 w-full rounded-full border border-[#ff7a1a] text-base font-semibold text-[#ff7a1a]">Confirm Your Pair</button>
                    </>
                  ) : null}
      ...
      {total > 0 ? (
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-[#dbdbdb] bg-[#efefef] p-3 pb-[max(env(safe-area-inset-bottom),12px)] dark:border-[#6a5791] dark:bg-[#3f2d63]">
          <div className="flex items-center gap-3">
            <div className="min-w-[110px]"><p className="text-sm text-[#666] dark:text-[#c0b7d9]">Total Amount:</p><p className="text-4xl font-bold leading-9 text-[#ff7a1a]"><span className="currency-inr">&#8377;</span>{total}</p></div>
            <Link href={routes.tournamentCheckout(id)} className="grid h-11 flex-1 place-content-center rounded-full bg-[#ff7a1a] text-xl font-semibold text-white">Claim Spot</Link>
          </div>
        </div>
      ) : null}
    </div>

...
const TopAppBar = () => (
  <div className="flex items-center justify-between">
    <Link href="/org/tournaments" className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-gray-800 border border-gray-100">
      <ArrowLeftIcon size={20} />
    </Link>
    <div className="flex gap-3">
      <button className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-gray-800 border border-gray-100"><ShareIcon size={18} /></button>
      <button className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-gray-800 border border-gray-100"><EllipsisIcon size={20} /></button>
    </div>
  </div>
);
...
const EventHeader = () => (
  <div className="flex gap-3 items-center">
    ...
    <div>
      <h1 className="font-semibold text-lg leading-tight text-gray-900">Mumbai Men&apos;s 2025</h1>
  ...
  <div className="grid grid-cols-2 gap-3">
    ...
    <div className="rounded-xl bg-white p-4 shadow-sm border border-gray-100">
      ...
      <div className="flex gap-2">
        ...
        <button className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs font-medium flex-1 transition-transform active:scale-95">Close</button>
      </div>
    </div>
  </div>
);
...
const PrimaryTabs = ({ tabs, activeTab, setActiveTab }: { tabs: string[]; activeTab: string; setActiveTab: (tab: string) => void }) => (
  <div className="flex justify-center my-2">
    <div className="flex gap-1 bg-gray-100 p-1 rounded-full overflow-x-auto scrollbar-hide max-w-full items-center">
      {tabs.map((tab) => (
        <button key={tab} onClick={() => setActiveTab(tab)} className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeTab === tab ? "bg-orange-500 text-white shadow-sm" : "text-gray-500 bg-transparent hover:text-gray-700"}`}>
          {tab}
        </button>
      ))}
    </div>
...
const AboutTab = () => (
  <>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="border border-gray-100 rounded-xl p-3 bg-gray-50/50 col-span-2 flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-gray-900 leading-snug">Andheri Sports Complex, Veera Desai Road, Mumbai</p>
          </div>
        </div>
      </div>
    </div>
    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mt-4">
      ...
      <p className="text-sm text-gray-600 leading-relaxed">Join the biggest Pickleball tournament in the city. Open to all men&apos;s and all city athletes. Experience state-of-the-art courts, competitive matches, and a chance to win amazing prizes.</p>
    </div>
    <div className="bg-white rounded-xl p-4 space-y-4 shadow-sm border border-gray-100 mt-4">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 text-sm text-gray-600 ml-1">
          <PhoneIcon size={16} className="text-gray-400 shrink-0" /><p>+91 99212 48196</p>
        </div>
        <div className="flex items-center gap-3 text-sm text-gray-600 ml-1">
          <MailIcon size={16} className="text-gray-400 shrink-0" /><p>Email2019123@gmail.com</p>
        </div>
      </div>
    </div>
  </>
...
const StepRow = ({ title, state, subtext, actionLabel, href, isLast = false }: any) => {
  ...
  const buttonClass = `px-3 py-1.5 rounded-full text-xs font-medium text-center transition-all inline-block ${isCompleted ? "bg-green-50 text-green-700 border border-green-200 hover:bg-green-100" : isActive ? "bg-orange-500 text-white hover:bg-orange-600 shadow-sm" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`;

  return (
    <div className="flex gap-3 relative">
      <div className="flex flex-col items-center w-5 shrink-0">
        <div className={`w-5 h-5 rounded-full flex items-center justify-center z-10 ${isCompleted ? "bg-green-500 text-white" : isActive ? "bg-orange-500 border-2 border-orange-500" : "bg-white border-2 border-gray-300"}`}>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        ...
        <div className="flex justify-between items-center mb-4">
          <button className="border border-gray-200 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">Extend Due Date</button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 mt-3 pt-2">
            <div><p className="text-xs text-gray-500 mb-0.5">Enrolled</p><p className="text-xl font-bold text-gray-900">64</p></div>
            <div><p className="text-xs text-gray-500 mb-0.5">Confirmed (Paid)</p><p className="text-xl font-bold text-gray-900">64</p></div>
          </div>
        </div>
        <div className="border-t border-gray-100 bg-gray-50/30">
          <button onClick={() => setIsExpanded(!isExpanded)} className="w-full py-3 flex items-center justify-center gap-1 text-xs font-medium text-gray-500 hover:text-gray-700 transition-colors">
          ...
          {isExpanded && (
            <div className="px-4 pb-4 space-y-2">
              <div className="flex justify-between items-start py-2 group cursor-pointer">
                <div className="flex gap-3"><div className="w-2 h-2 rounded-full mt-1.5 shrink-0 bg-red-500" /><div><p className="font-medium text-sm text-gray-900">Fixtures Setup Remaining</p><p className="text-xs text-gray-500 mt-0.5">Round 2 pending generation</p></div></div>
                <ChevronRightIcon size={16} className="text-gray-400 transition-transform group-hover:translate-x-1" />
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex border-b border-gray-200 mb-4">
        <button onClick={() => setActiveRole("admin")} className={`flex-1 pb-2 text-center text-sm font-medium transition-colors ${activeRole === "admin" ? "text-orange-500 border-b-2 border-orange-500" : "text-gray-500 hover:text-gray-700"}`}>Admins</button>
        <button onClick={() => setActiveRole("scorer")} className={`flex-1 pb-2 text-center text-sm font-medium transition-colors ${activeRole === "scorer" ? "text-orange-500 border-b-2 border-orange-500" : "text-gray-500 hover:text-gray-700"}`}>Scorers</button>
      </div>
      <form onSubmit={(e) => e.preventDefault()} className="space-y-2 mb-6">
        <p className="font-medium text-gray-900 text-sm">Add {activeRole === "admin" ? "Admin" : "Scorer"}</p>
        <div className="flex gap-2">
          <input type="tel" placeholder="Enter phone number" className="flex-1 rounded-xl bg-white border border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all shadow-sm" />
      ...
      <div className="space-y-3">
        {displayedCrew.map((member) => (
          <div key={member.id} className="flex items-center justify-between bg-white rounded-xl p-3 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-sm font-semibold text-gray-600 shrink-0">{member.initials}</div>
...
export default function TournamentEventDetailsPage() {
  ...
  // Default tab when user views the Tournament Page
  ...
  return (
    <div className="min-h-screen bg-[#F6F6F6] px-4 py-3 pb-24 space-y-4">
      ...
      <PrimaryTabs tabs={primaryTabs} activeTab={activeTab} setActiveTab={setActiveTab} />

...
export default function OrgEventFixturesPage() {
  const router = useRouter();
  const params = useParams();

  return (
    <Layout showBottomNav={false} title="Fixtures">
      <div className="p-4 space-y-4">
        ...
        <div className="p-4 rounded-[var(--radius-card)] bg-[var(--color-surface)] border border-[var(--color-border)] shadow-[var(--shadow-card)] space-y-1">
          <p className="font-medium">Event fixtures</p>
          <p className="text-sm text-[var(--color-muted)]">
            Tournament: {String(params.id)} · Event: {String(params.eventId)}
          </p>
          <p className="text-sm text-[var(--color-muted)]">
            Placeholder page for bracket/fixtures.
          </p>
        </div>
      </div>
    </Layout>

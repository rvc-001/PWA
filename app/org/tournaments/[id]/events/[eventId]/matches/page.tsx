...
export default function OrgEventMatchesPage() {
  const router = useRouter();
  const params = useParams();

  return (
    <Layout showBottomNav={false} title="Matches">
      <div className="p-4 space-y-4">
        ...
        <div className="p-4 rounded-[var(--radius-card)] bg-[var(--color-surface)] border border-[var(--color-border)] shadow-[var(--shadow-card)] space-y-1">
          <p className="font-medium">Event matches</p>
          <p className="text-sm text-[var(--color-muted)]">
            Tournament: {String(params.id)} · Event: {String(params.eventId)}
          </p>
          <p className="text-sm text-[var(--color-muted)]">
            Placeholder page for match list/scoring access.
          </p>
        </div>
      </div>
    </Layout>
